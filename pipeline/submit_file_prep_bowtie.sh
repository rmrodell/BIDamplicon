...
usage() {
    echo "Usage: $0 -u <email> -s <script> -a <map> -i <input-dir> -o <out-dir> -b <bowtie-index> [OPTIONS]..."
    echo ""
    ...
    echo "  -i, --input-dir <path>       Path to the directory containing the source FASTQ files."
    ...
    echo "      --input-dir /path/to/fastq_files/ \\"
...
while [[ $# -gt 0 ]]; do
    ...
    case $key in
        ...
        -i|--input-dir)         INPUT_SOURCE_DIR="$2"; shift 2 ;;
...
if [ -z "$INPUT_SOURCE_DIR" ]; then missing_args+=("--input-dir"); fi
if [ -z "$TOP_LEVEL_OUTPUT_DIR" ]; then missing_args+=("--output-dir"); fi
if [ -z "${BOWTIE2_INDEX}.1.bt2" ]; then missing_args+=("--bowtie-index"); fi
...
if [ ! -d "$INPUT_SOURCE_DIR" ]; then echo "Error: FASTQ source directory '$INPUT_SOURCE_DIR' not found."; exit 1; fi
if [ ! -f "${BOWTIE2_INDEX}.1.bt2" ]; then echo "Error: Bowtie2 index file not found. Checked for: ${BOWTIE2_INDEX}.1.bt2"; exit 1
fi
...
echo "  FASTQ Source Dir: $INPUT_SOURCE_DIR"
echo "  Output Dir:       $TOP_LEVEL_OUTPUT_DIR"
echo "  Bowtie Index:     $BOWTIE2_INDEX"
echo "---------------------------------------------"
...
sbatch \
    ...
    "$PIPELINE_SCRIPT_PATH" --map-file "$SAMPLE_MAP_FILE" --input-dir "$INPUT_SOURCE_DIR" --output-dir "$TOP_LEVEL_OUTPUT_DIR" --bowtie-index "$BOWTIE2_INDEX"
